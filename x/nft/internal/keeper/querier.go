...
func querySupply(ctx sdk.Context, path []string, req abci.RequestQuery, k Keeper) ([]byte, sdk.Error) {
	...
	if err != nil {
		return nil, sdk.ErrUnknownRequest(fmt.Sprintf("incorrectly formatted request data %v", err.Error()))
	}

	collection, found := k.GetCollection(ctx, params.Denom)
	if !found {
		return nil, sdk.ErrUnknownRequest( fmt.Sprintf("unknown denom %s", params.Denom))
	}
...
func queryNFT(ctx sdk.Context, path []string, req abci.RequestQuery, k Keeper) ([]byte, sdk.Error) {
	...
	if err != nil {
		return nil, sdk.ErrUnknownRequest( fmt.Sprintf("invalid NFT #%s from collection %s", params.TokenID, params.Denom))
	}
